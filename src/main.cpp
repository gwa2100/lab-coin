...
#if defined(NDEBUG)
# error "Labcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x271abf1a867a23d4fb00752adea46cf94945fee234f1fac30cd314673b8f3215");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Labcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Labcoin Signed Message:\n";
...
{
    // Labcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Labcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 7.5; // Labcoin: 7.5 minutes
static const int64 nTargetSpacing = 1 * 15; // Labcoin: 15 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Labcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Labcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x271abf1a867a23d4fb00752adea46cf94945fee234f1fac30cd314673b8f3215");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Rueters 22/Nov/2017 US Calls Myanmar moves against Rohingya ethnic cleansing.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040112310fa689ad5023690c80f1239c8fbad8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1511403440;
        ...
        assert(block.hashMerkleRoot == uint256("0x9eb929382e39a8345c1789e92839b79595d6a16570f525edc06e5e4b4e4340b4"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Labcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LabcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LabcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LabcoinMiner(CWallet *pwallet)
{
    printf("LabcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("labcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LabcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LabcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LabcoinMiner, pwallet));
}
